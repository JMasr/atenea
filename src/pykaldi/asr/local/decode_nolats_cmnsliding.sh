...
if [ $# != 4 ]; then
   echo "Usage: $0 [options] <graph-dir> <data-dir> <models-dir> <decode-dir>"
   echo "This version produces just linear output, no lattices"
   echo ""
   echo "e.g.: steps/decode.sh exp/mono/graph_tgpr data/test_dev93 exp/mono exp/mono/decode_dev93_tgpr"
   echo ""
...
for f in $sdata/1/feats.scp $sdata/1/cmvn.scp $model $graphdir/HCLG.fst; do
  [ ! -f $f ] && echo "decode_nolats_cmnsliding.sh: no such file $f" && exit 1;
done

if [ -f $srcdir/final.mat ]; then feat_type=lda; else feat_type=delta; fi
echo "decode_nolats_cmnsliding.sh: feature type is $feat_type";
...
case $feat_type in
  delta) feats="ark,s,cs:apply-cmvn-sliding $cmvn_opts  --norm-vars=false --center=true --cmn-window=300 scp:$sdata/JOB/feats.scp ark:- | add-deltas $delta_opts ark:- ark:- |";;
  lda) feats="ark,s,cs:apply-cmvn-sliding $cmvn_opts --norm-vars=false --center=true --cmn-window=300 scp:$sdata/JOB/feats.scp ark:- | splice-feats $splice_opts ark:- ark:- | transform-feats $srcdir/final.mat ark:- ark:- |";;
