...
if [ -f path.sh ]; then . ${dirRelative}/conf/path.sh; fi
. ${dirRelative}/conf/parse_options.sh || exit 1;
...
${dirRelative}/utils/split_scp.pl $utt2spk_opt ${listaFicheros} $feats || exit 1

for n in `seq $nj`; do
   vads="$vads $data/split$nj/$n/vad.scp"
done

${dirRelative}/utils//split_scp.pl $utt2spk_opt ${listaVAD} $vads || exit 1
